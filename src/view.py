...
class TaskView:
    ...
    def show_tasks_screen(self, username):
        ...
        self.btn_add = tk.Button(self.main_frame, text="Adicionar Tarefa", command=self.controller.add_task, bg="#4CAF50", fg="white", font=self.custom_font, relief="flat", width=15)
        self.btn_add.pack(side=tk.LEFT, padx=30, pady=10)

        self.btn_remove = tk.Button(self.main_frame, text="Remover Tarefa", command=self.controller.remove_task, bg="#f44336", fg="white", font=self.custom_font, relief="flat", width=15)
        self.btn_remove.pack(side=tk.LEFT, padx=20, pady=10)

        self.btn_clear = tk.Button(self.main_frame, text="Limpar Tarefas", command=self.controller.clear_tasks, bg="#FF9800", fg="white", font=self.custom_font, relief="flat", width=15)
        self.btn_clear.pack(side=tk.LEFT, padx=20, pady=10)

        self.btn_edit = tk.Button(self.main_frame, text="Editar Tarefa", command=self.controller.edit_task, bg="#2196F3", fg="white", font=self.custom_font, relief="flat", width=15)
    ...
    def get_selected_task_index(self):
        return self.task_listbox.curselection()[0] if self.task_listbox.curselection() else None